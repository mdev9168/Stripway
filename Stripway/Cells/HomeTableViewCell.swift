...
class HomeTableViewCell: UITableViewCell,TagListViewDelegate {
    ...
    func updateView() {
        ...
        if self.post?.caption == "" && self.post?.caption == nil {
            ...
            for text in arrObj{
                ...
                captionText = post?.caption as! String
                tagArr = post?.hashTags as! [String]
        ...
        if captionText != "" && tagArr.count != 0 {
            ...
            print("caption  found")
            self.myScrollView.contentOffset.x = SCREEN_WIDTH//show caption
            self.viewHashTagContnr.isHidden = true
            self.viewCaptionContner.isHidden = false
    ...
    func formatUsernameLabel() {
        if let post = post, let user = user {
            ...
            let popAttributedString = NSMutableAttributedString(string: boldText, attributes: popAttrs)
    ...
    @objc func commentButtonPressed() {
     ...
     func setPopUpConstraint(_ point:CGPoint) {
       ...
       func movingShowPopUpMenu(_ point:CGPoint) {
           ...
           if dmDistance < effectRadius {
               if self.shakingPopup == false {
                   self.shakeDMButton(point, isDMIncrease)
               }
           }
           
           if shareDistance < effectRadius {
               if self.shakingPopup == false {
                   self.shakeShareButton(point, isShareIncrease)
               }
           }
           
           if bookDistance < effectRadius {
               if self.shakingPopup == false {
                   self.shakeBookButton(point, isBookIncrease)
               }
           }
           
           if repostDistance < effectRadius {
               if self.shakingPopup == false {
                   self.shakeRepostButton(point, isRepostIncrease)
               }
           }
       ...
       @objc func showPopUp(sender: UILongPressGestureRecognizer) {
           let point:CGPoint = sender.location(in: self.postImageView)
