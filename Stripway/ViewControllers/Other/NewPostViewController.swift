...
//            lblStripNotFound.isHidden = false
...
extension NewPostViewController:AddTagViewControllerDelegate{
    ...
    func getRectBounds()->CGRect{
        let width = Int(imageToPostView.bounds.width) - 200
        let height = Int(imageToPostView.bounds.height) - 40
        let originY = Int(scrollView.frame.origin.y)
        let x = Int.random(in: 0...width)
        let y = Int.random(in: 0...height) + originY
    ...
    func getVaildBounds(center: CGPoint)->Bool{
        let minX = 40.0
        let minY = scrollView.frame.origin.y + 20.0
        let maxX = UIScreen.main.bounds.width - 40
        let maxY = minY + imageToPostView.bounds.size.height - 40
        
        let x = center.x
        let y = center.y
        
        if (x > CGFloat(minX)) && (x < maxX) && (y > minY) && (y < maxY) {
            return  true
        }
        return false
    }
