...
class ViewConversationViewController: MessagesViewController {
...
//    248DE1
    ...
    @objc func showImage(forImageURL imageURL:URL) {
        ...
        KingfisherManager.shared.retrieveImage(with: imageURL) { result in
            ...
            if let image = result.value?.image{
                ...
                self.presentImageVC(imageVC: photosVC)
            }
        }
                ...
                if let message = message as? StripwayMessage {
                    ...
                    API.Post.observePost(withID: postID!) { (post, error) in
                        ...
                        if let error = error {
                            ...
                            let normalString = NSMutableAttributedString(string: post.caption, attributes: attrs2)
                    ...
                    API.User.observeUser(withUID: author!) { (user, error) in
                        if let error = error {
                            ...
                            let attributedString = NSMutableAttributedString(string: user.username, attributes: attrs)
...
extension ViewConversationViewController: MessageCellDelegate {
    func didTapMessage(in cell: MessageCollectionViewCell) {
        ...
        if let indexPath = messagesCollectionView.indexPath(for: cell) {
            let message = messages[indexPath.section]
            if message.type == .image {
                let imageURL = URL(string: message.photoURL!)
                self.showImage(forImageURL: imageURL!)
            }
            if message.type == .post {
...
extension ViewConversationViewController: MessageInputBarDelegate {
