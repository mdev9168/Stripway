...
//        slideshowView.pageIndicator = nil
    ...
    func fetchFeaturedList() {
        ...
        self.featuredPostUsers.removeAll()
        self.featuredTimestamps.removeAll()
        self.btnFeatured.isUserInteractionEnabled = false
        
        API.User.usernamesReference.child("stripway").child("uid").observeSingleEvent(of: .value) { (snapshot) in
            ...
            if snapshot.exists() {
                ...
                if let adminId = snapshot.value as? String {
                    ...
                    API.Reposts.getRepostsFeed(withID: adminId) { (results) in

                        if results.count > 0 {
                            results.forEach({ (result) in
                                print("loading repost: \(result.2)")
                                self.featuredPosts.append(result.0)
                                self.featuredPostUsers.append(result.1)
                                self.featuredTimestamps.append(result.2)
                            })
                        }
...
extension TrendingViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row >= featuredPosts.count {
            //to resolve crash when overflow
            return cell
        }
        cell.index = indexPath.row
        cell.post = featuredPosts[indexPath.row]
        cell.user = featuredPostUsers[indexPath.row]
        cell.delegate = self
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.row >= self.featuredPosts.count {
            return
        }        
        self.goToPostVC(post: self.featuredPosts[indexPath.row], posts: [self.featuredPosts[indexPath.row]])
    }
}

extension TrendingViewController: FeatureCollectionViewCellDelegate {
    func segueToProfileFor(_ index: Int) {
        if self.featuredPostUsers.count > index {
            self.tappedUser = self.featuredPostUsers[index]
